...
process :: [Matched] -> Registers -> Registers
process [] registers = registers
process (x:xs) registers = let 
  ...
  in 
    ...
    then process xs (updateRegisters registers (register x) regVal (operation x) (value x))
    else process xs registers

initProcess :: [Matched] -> Registers
initProcess steps = process steps Map.empty 

getFinalState :: String -> Registers
getFinalState input = initProcess $ parse $ lines input

mapMaxValue :: Registers -> Int
mapMaxValue registers = if Map.null registers then 0 else maximum $ map snd $ Map.toList registers

solve input = mapMaxValue $ getFinalState input
