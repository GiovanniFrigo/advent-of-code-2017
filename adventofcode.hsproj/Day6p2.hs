...
-- once we find the loop, get how many positions ago it was
-- luckily I used a list and not an HashSet :D
parse' :: [Int] -> [String] -> Int
parse' state alreadySeenDispositions = 
  if currentDisposition `elem` alreadySeenDispositions
  then (length alreadySeenDispositions) - (fromJust $ findIndex (\x -> currentDisposition == x) alreadySeenDispositions)
  else (parse' (doStep state) (alreadySeenDispositions ++ [currentDisposition]))  
