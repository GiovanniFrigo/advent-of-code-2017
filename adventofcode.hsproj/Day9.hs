...
parse ('<':xs) openedGroups garbageMode = 0 + parse xs openedGroups True -- enterning garbage (if not already inside it)
parse ('>':xs) openedGroups garbageMode = 0 + parse xs openedGroups False -- exiting garbage
